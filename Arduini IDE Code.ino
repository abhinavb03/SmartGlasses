...
const char* ble_name = "ESP32 Smart Glasses";
...
int _TouchSensorGPIO = 27;
int _TouchSensorThreshold = 20;
int _PowerSaveInterval = 200000;

float _BatteryVoltageFlat       = 3.7f; // In my case 3.1V from ADC means ~3.6V from battery (/ ADC in ESP is not linear! so we cant calculate exact value of battery just estimate!)
...
class ButtonClass {
  ...
  int detect(int _TouchSensorGPIO) {    
    long now = millis();
    int btnState = (touchRead(_TouchSensorGPIO) < _TouchSensorThreshold) ? 1 : 0;
    ...
    class BLEReceiveClass : public BLECharacteristicCallbacks {
        ...
        void onWrite(BLECharacteristic *pCharacteristic) {
          ...
          if (receivedValue.length() > 0) {
            ...
            else if (sReceived.startsWith("#TSG=" ))  { _TouchSensorGPIO      =sReceived.substring(5, sReceived.length()).toInt(); Serial.println(_TouchSensorGPIO)     ; return; }  // TouchSensorGPIO      Def: 27
...
void setup() {
  ...
  // Info OLED
  delay(100);
  dc.setFontSize(7); dc.drawString(10, 10, "Ready, Mode: " ); dc.drawString(10, 20, (getOperatingMode() == WEB_UPDATE) ? "WebUpdate" : "Bluetooth" ); dc.sendBuffer(); 
...
int getBatteryStatus() {
  ...
  voltage = ( 4095.0f / 4095.0f ) * 3.8; //analogRead(BATTERY_GPIO)
...
void prepareToDeepSleep() {
  ...
  adc_power_off();  //__attribute__((deprecated)); 
}
